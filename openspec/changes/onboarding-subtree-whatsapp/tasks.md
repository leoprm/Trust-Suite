...
- [ ] 1.1 Agregar `whatsappGroupId String?` al modelo Tree en schema.prisma + migración
- [ ] 1.2 Reescribir el segundo mensaje de onboarding (`bot/index.ts` ~línea 449):
...
- [ ] 1.3 Modificar `handleOnboardingResponse` para manejar el flujo multi-step:
...
- [ ] 1.4 Agregar estado de onboarding en `BotSessionData`: `onboardingStep`, `onboardingTreeId`

## 2. Locales + Validación

- [ ] 2.1 Agregar textos a `locales/es.json` y `locales/en.json`:
...
- [ ] 2.2 Validar código del árbol padre: buscar por código en DB, mostrar nombre si existe

## 3. Tests

- [ ] 3.1 Test: nuevo árbol independiente → flujo completo (objetivos → no subárbol → whatsapp)
- [ ] 3.2 Test: subárbol → ingresa código padre válido → linkeado correctamente
- [ ] 3.3 Test: subárbol → código padre inválido → mensaje de error y re-pregunta
- [ ] 3.4 Test: /skip en WhatsApp → campo queda null
