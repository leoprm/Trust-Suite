  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setError(err?.response?.data?.error || 'Error al cargar costo');
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className="glass-panel" style={{ padding: isMobile ? '1.5rem' : '2rem' }}>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Consultando costo…</p>
        </div>
      )}

      {error && !loading && (
        <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
          <AlertCircle size={24} style={{ color: 'var(--accent-danger)', marginBottom: '0.5rem' }} />
        ...
        <>
          <div style={{
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', marginTop: '0.5rem' }}>
              Incluye infraestructura, IA, y crecimiento de la plataforma
            </p>
          </div>
